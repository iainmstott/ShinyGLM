...
### 'PERTURBATION ANALYSIS' PANEL
    ...
    # # render a line plot of survival over age
    # output$survageplot <- plotOutput({
    #     plot(LT()$qx, bty = "n", xlab = "", ylab = "",
    #          lwd = 1.5, cex.axis = 1.2, col = "darkred")
    #     mtext(side = 1, line = 2.5, cex = 1.2,
    #           "Age")
    #     mtext(side = 2, line = 2.5, cex = 1.2
    #           "Mortality")
    ...
    # lifeTable <- reactive({
    #     input$modelUpdate
    #     LT <- makeLifeTable(matU = isolate(Asplit()$matU),
    #                         matF = isolate(Asplit()$matF),
    #                         matC = isolate(Asplit()$matC))
    #     LT
    # })
